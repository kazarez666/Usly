...
const secretPhotoUrlCache=new Map<string,{url:string|null;expiresAt:number}>()
const secretPhotoRequests=new Map<string,Promise<{ok:boolean;photos:SecretPhoto[];error?:string}>>()
...
async function getSecretPhotoUrl(path:string){
  if(!supabase)return null
  const cached=secretPhotoUrlCache.get(path)
  if(cached&&cached.expiresAt>Date.now())return cached.url
  const signed=await supabase.storage.from('secret-media').createSignedUrl(path,3600)
  const url=signed.error?null:signed.data?.signedUrl??null
  if(!signed.error)secretPhotoUrlCache.set(path,{url,expiresAt:Date.now()+SIGNED_URL_CACHE_MS})
  return url
}
...
async function fetchSecretPhotos(coupleId:string):Promise<{ok:boolean;photos:SecretPhoto[];error?:string}>{
  if(!supabase)return{ok:false,photos:[],error:'SUPABASE_MISSING'}
  ...
  const source=rows<any>(data)
  const urls=await Promise.all(source.map(r=>r.path?getSecretPhotoUrl(r.path):Promise.resolve(null)))
  const photos=source.map((r,index)=>({id:r.id,coupleId:r.couple_id,uploadedBy:r.uploaded_by,path:r.path,caption:r.caption??'',createdAt:r.created_at,url:urls[index]}))
  return{ok:true,photos}
}

export function getSecretPhotos(coupleId:string){
  const current=secretPhotoRequests.get(coupleId)
  if(current)return current
  let request:Promise<{ok:boolean;photos:SecretPhoto[];error?:string}>
  request=fetchSecretPhotos(coupleId).finally(()=>{if(secretPhotoRequests.get(coupleId)===request)secretPhotoRequests.delete(coupleId)})
  secretPhotoRequests.set(coupleId,request)
  return request
}
export async function uploadSecretPhoto(coupleId:string,file:File,caption=''){
  ...
  const userId=await getSessionUserId(); if(!userId) return {ok:false,error:'NOT_AUTHENTICATED'}
...
export async function deleteSecretPhoto(id:string,path:string){
  ...
  await supabase.storage.from('secret-media').remove([path]); secretPhotoUrlCache.delete(path); return {ok:true}
}
